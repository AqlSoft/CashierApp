...
<h2 class="mt-4 pb-2" style="border-bottom: 1px solid #dedede">
  <i class="fa fa-edit px-1"></i> {{ __('sessions.update_sales_session') }}
</h2>
<fieldset class="mt-2 mb-4 mx-0 mb-0 shadow-sm">

  <div class="row">
    <div class="card-body">
      <form action="{{ route('update-sales-session-info') }}" method="POST">
        <input type="hidden" name="id" value="{{ $sales_session->id }}">
        @csrf
        <div class="input-group sm mb-2">
          <label class="input-group-text" for="start_time">{{ __('sessions.start_time') }}</label>
          <input type="text" class="form-control sm" name="start_time" id="start_time" value="{{ old('start_time', $sales_session->start_time) }}">
          ...
          <select class="form-select form-control sm py-0" name="admin_id" id="admin_id">
            ...
            <option value="{{ $admin->id }}" {{ $sales_session->admin_id == $admin->id ? 'selected' : '' }}>
              {{ $admin->userName }}
              @if($admin->activeSalesSession)
        ...
        <div class="input-group sm mt-2">
          ...
          <select class="form-select form-control sm py-0" name="monybox_id" id="monybox_id" required>
            ...
            <option value="{{ $monyBox->id }}" {{ $sales_session->monybox_id == $monyBox->id ? 'selected' : '' }}>
              {{ $monyBox->name }}
              @if($monyBox->activeSalesSession)
                {{ __('sessions.box_has_active_sales_session') }}
              @endif
            </option>
            @endforeach
          </select>
          <label class="input-group-text" for="notes">{{ __('sessions.notes') }}</label>
          <input type="text" class="form-control sm" name="notes" id="notes" value="{{ old('notes', $sales_session->notes) }}">

          <label class="input-group-text" for="opening_balance">{{ __('sessions.opening_balance') }}</label>
          <input type="number" class="form-control sm" name="opening_balance" id="opening_balance" value="{{ old('opening_balance', $sales_session->opening_balance) }}">
