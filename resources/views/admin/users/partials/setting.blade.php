    ...
    <div class="d-flex justify-content-start align-items-start setting-nav p-0">
      ...
      <div class="nav flex-column justify-content-start text-end" style="width:180px;" id="v-pills-tab" role="tablist" aria-orientation="vertical">
        ...
        <button class="nav-link" id="v-pills-messages-tab" data-bs-toggle="pill" data-bs-target="#v-pills-messages" type="button" role="tab" aria-controls="v-pills-messages" aria-selected="false">
          {{ __('profile.internal_communication') }}
        </button>
      ...
      <div class="tab-content p-3 m-0" id="v-pills-tabContent">
        ...
        <div class="tab-pane fade show active" id="v-pills-password" role="tabpanel" aria-labelledby="v-pills-password-tab" tabindex="0">
          ...
          <form method="POST" action="{{ route('admins.updatePassword', $admin->id) }}">
            ...
            @method('PUT')
            <div class="row mb-3">
              <div class="col col-5 fw-bold text-end">{{ __('profile.current_password') }}:</div>
              <div class="col col-7">
                <input type="password" class="form-control" name="current_password" required>
              </div>
            </div>
            <div class="row mb-3">
              <div class="col col-5 fw-bold text-end">{{ __('profile.new_password') }}:</div>
              <div class="col col-7">
                <input type="password" class="form-control" name="new_password" required>
              </div>
            </div>
            <div class="row mb-3">
              <div class="col col-5 fw-bold text-end">{{ __('profile.confirm_password') }}:</div>
              <div class="col col-7">
        ...
        <div class="tab-pane fade" id="v-pills-notifications" role="tabpanel" aria-labelledby="v-pills-notifications-tab" tabindex="0">
          ...
          <form method="POST" action="">
            ...
            <div class="form-check">
              <input class="form-check-input" type="checkbox" name="enable_notifications" id="enable_notifications" {{ $admin->profile->enable_notifications ? 'checked' : '' }}>
              <label class="form-check-label" for="enable_notifications">
                {{ __('profile.enable_notifications') }}
              </label>
            </div>
            <button type="submit" class="btn btn-primary mt-3">{{ __('profile.save_changes') }}</button>
          </form>
        </div>
