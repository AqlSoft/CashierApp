                        ...
                        <tbody>
                            ...
                            <tr>
                                <form action="">
                                    ...
                                    <td>
                                        <a class="btn btn-sm py-0" onclick="if(!confirm('You are about to delete a item, are you sure!?.')){return false}"
                                            title="delete order and related Information" href="{{ route('destroy-oItem-info', $oItem->id) }}">
...
=======
    <h1 class="mt-3 pb-2" style="border-bottom: 1px solid #dedede">
        <a href="{{ route('fast-creqate-order') }}">New</a>Add Order Items
    </h1>
    <div class="row gap-1">
        @forelse ($orders as $pendingOrder)
            <a href="{{ route('add-orderitem-input-entry', [$pendingOrder->id]) }}"
                class="col col-auto btn btn-outline-primary py-0">{{ $pendingOrder->wait_no }}</a>
        @empty
        @endforelse
    </div>
    <div class="container">
                ...
                <div class="row">
                    <div class="col col-12">
                        ...
                        <select class="form-select form-control mt-2" id="category-select">
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}">{{ $category->cat_name }}</option>
>>>>>>> f96f91a1830dec77465ed15849d25f7c072b1784
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="row d-flex gap-2" id="product-list">
                    @if (isset($products) && count($products))
                        @foreach ($products as $product)
                            <div class="col col-2 productlist"
                                style="border: 1px solid {{ in_array($product->id, $Ois) ? '#007bff' : '#f7f5f5' }}"
