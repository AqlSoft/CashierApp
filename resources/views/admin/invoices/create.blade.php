...
<fieldset class="table mt-3">
  <form action="{{ route('save-invoices-orderitem-info') }}" method="post" class="bg-transparent p-0 ">
    @csrf
      ...
      <tfoot class="bg-transparent mt-3">
        <tr>
          ...
          <td class="text-primary">
            <input id="amount" name="amount" class="bg-transparent" value="{{ $amount }}"  style="border:none;border-bottom: 2px solid #dedede" readonly>
          </td>
        </tr>
        <tr>
          <td colspan="6" class="fw-bold text-end text-primary ">Vat Amount :</td>
          <td class="text-primary">
            <input id="vat_amount" name="vat_amount" class="bg-transparent" value="{{ $vatAmount }}" style="border:none;border-bottom: 2px solid #dedede">
          </td>
        </tr>
        ...
        <tr>
          ...
          <td class="text-primary">
            <input id="total_amount" name="total_amount" class="bg-transparent" value="{{ $totalAmount }}" style="border:none;border-bottom: 2px solid #dedede" readonly>
          </td>
        </tr>
      </tfoot>
    </table>
      ...
      <div class="modal-body">
        <form action="{{ route('payments.cash.store', $order->id) }}" method="POST">
          ...
          <div class="input-group sm mb-2">
            ...
            <input type="number" class="form-control sm" value="{{ $amount }}" name="amount" id="amount">
          </div>
          <div class="input-group sm mb-2">
            <label class="input-group-text" for="vatAmount">Vat Amount </label>
            <input type="number" class="form-control sm" name="vat_amount" value="{{ $vatAmount }}" id="vatAmount">
          </div>
          <div class="input-group sm mb-2">
            <label class="input-group-text" for="total_amount">Total Amount</label>
            <input type="number" class="form-control sm" value="{{$totalAmount}}" name="total_amount" id="total_amount">
          </div>
          <div class="input-group sm mb-2">
